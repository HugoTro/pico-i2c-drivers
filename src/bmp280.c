...
int16_t bmp280_calibration_settings[12] = {0};
...
int bmp280_get_calibration_settings(uint16_t *ptr) {
	if (initialised==0) {
		return -1;
	}
	memcpy(ptr, bmp280_calibration_settings, 24 /*(12*sizeof(uint16_t))/sizeof(uint8_t)*/);
	return 0;
}
...
int bmp280_read_temp(uint32_t *temp) {
	*temp = 0;
	...
	printf("\t%d\n", *temp);
	// Need to create a temp variable for the unsigned value, that is stored in a signed array
	uint16_t calib_us = 0;
	memcpy(&calib_us, bmp280_calibration_settings, sizeof(short));
	// Convert meaningless value to temperature
		// double var1 = ((*temp)/16384.0-27504/1024.0)*((double)26435);
	double var1 = ((*temp)/16384.0-calib_us/1024.0)*((double)bmp280_calibration_settings[1]);
		printf("\tvar1: %f\n", var1);
	double var2 = ((*temp)/131072.0-calib_us/8192.0)*((*temp)/131072.0-calib_us/8192.0)*((double)bmp280_calibration_settings[2]);
